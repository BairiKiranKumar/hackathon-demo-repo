...
export const Member: MemberRelationResolvers = {
  insight: (_obj, { root }) => {
    return db.member.findUnique({ where: { id: root?.id } }).insight()
  },
  overview: (_obj, { root }) => {
    return db.member.findUnique({ where: { id: root?.id } }).overview()
  },
  encounter: (_obj, { root }) => {
    return db.member.findUnique({ where: { id: root?.id } }).encounter()
  },
}
